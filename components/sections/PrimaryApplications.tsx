...
const applications = [
  {
    ...
    description: 'Enable sophisticated structured finance products with stable, predictable funding costs for institutional borrowers.',
    features: ['Credit facilities', 'Asset-backed securities', 'Trade finance'],
  },
  {
    icon: faChartBar,
    title: 'Credit Markets',
    description: 'Power on-chain credit markets with reliable liquidity infrastructure and transparent risk assessment.',
    features: ['Lending protocols', 'Credit scoring', 'Risk management'],
  },
  {
    icon: faHandshake,
    title: 'Real-World Assets',
    description: 'Bridge traditional assets to DeFi with stable funding mechanisms for real-world asset tokenization.',
    features: ['Asset tokenization', 'Yield generation', 'Institutional custody'],
  },
  {
    icon: faGlobe,
    title: 'Cross-Border Payments',
    description: 'Facilitate efficient cross-border transactions with minimal volatility risk and predictable settlement.',
  ...
  return (
    <section className="py-20 bg-usdu-bg">
      <div className="container mx-auto px-4">
          ...
          {applications.map((application, index) => (
            ...
            >
              <div className="flex items-start gap-6">
                <div className="w-16 h-16 bg-usdu-orange/10 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:bg-usdu-orange/20 transition-colors">
                  <FontAwesomeIcon icon={application.icon} className="w-8 h-8 text-usdu-orange" />
                </div>
                <div className="flex-1">
                  <h3 className="text-xl font-bold text-usdu-black mb-4">{application.title}</h3>
                  <p className="text-text-secondary mb-6 leading-relaxed">{application.description}</p>
                  
                  {/* Features List */}
                  <ul className="space-y-2">
                    {application.features.map((feature) => (
                      <li key={feature} className="flex items-center gap-3 text-sm text-text-secondary">
                        <div className="w-1.5 h-1.5 bg-usdu-orange rounded-full flex-shrink-0" />
                        {feature}
                      </li>
                    ))}
                  </ul>
        ...
        >
          <div className="bg-gradient-to-r from-usdu-orange/5 to-usdu-orange/10 rounded-2xl p-8">
            <h3 className="text-2xl font-bold text-usdu-black mb-4">
              Ready to Build with USDU?
            </h3>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <a
                href="/transparency"
                className="inline-flex items-center justify-center gap-2 border border-usdu-orange text-usdu-orange px-6 py-3 rounded-xl hover:bg-usdu-orange hover:text-white transition-colors font-medium"
              >
                View Documentation
              </a>
            </div>
